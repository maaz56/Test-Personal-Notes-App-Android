...
final public class GDActions {
    ...
    static DriveId create(DriveId prId, String titl, String mime, byte[] buf) {
        ...
        if (buf != null) {
            ...
            meta = new MetadataChangeSet.Builder().setTitle(titl).setMimeType(GDUT.MIME_FOLDER).build();
    ...
    static DriveId createTreeGDAA(String root, String titl, byte[] buf) {
        ...
        if (dId != null) {
            dId = findOrCreateFolder(dId, GDUT.title2Month(titl));
            if (dId != null) {
    ...
    private static DriveId findOrCreateFolder(DriveId prnt, String titl) {
        ArrayList<GF> gfs = search(prnt, titl, GDUT.MIME_FOLDER);
        if (gfs.size() > 0) {
    ...
    static String create(String prId, String titl, String mime, byte[] buf) {
        ...
        if (titl != null && isConnected()) {
            ...
            if (buf != null) {
                if (mime != null) {
                    ...
                    try {
                        jvFl = GDUT.bytes2File(buf,
                                java.io.File.createTempFile(GDUT.TEMP_FILENM, null, GDUT.acx.getCacheDir()));
                ...
                meta.setMimeType(GDUT.MIME_FOLDER);
                try {
