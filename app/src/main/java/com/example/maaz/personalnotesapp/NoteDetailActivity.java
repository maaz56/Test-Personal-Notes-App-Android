...
public class NoteDetailActivity extends BaseActivity
                ...
                do {
                    ...
                    switch (storageSelection) {
                        case AppConstant.GOOGLE_DRIVE_SELECTION:
                            updateStorageSelection(null, R.drawable.google_drive, AppConstant.GOOGLE_DRIVE_SELECTION);
                            break;
    ...
    private void setValues(Note note) {
        ...
        mTitleEditText.setText(title);
        int storageSelection = note.getStrogeSelection();
        if (mIsList) {
            initializeComponents(LIST);
            CardView cardView = (CardView) findViewById(R.id.card_view);
            cardView.setVisibility(View.GONE);
    ...
    private void updateStorageSelection(Bitmap bitmap, int storageSelectionResource, int selection) {
        ...
        AppSharedPreferences.setPersonalNotesPreferences(getApplicationContext(), selection);
    }
    ...
    private void initializeComponents(int choice) {
        ...
        if (AppSharedPreferences.getUploadPreference(getApplicationContext()) == AppConstant.GOOGLE_DRIVE_SELECTION) {
            ...
            mStorageSelection.setImageResource(R.drawable.ic_local);
        }
        ...
        sDateTextView = (TextView) findViewById(R.id.date_text_view_make_note);
        sTimeTextView = (TextView) findViewById(R.id.time_text_view_make_note);
        ...
        datePickerImageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                datePicker.show(getSupportFragmentManager(), AppConstant.DATE_PICKER);
            }
        });
    ...
    private void setAlarm(Calendar targetCal, Note note) {
        Intent intent = new Intent(getBaseContext(), AlarmRecevier.class);
        intent.setFlags(intent.FLAG_ACTIVITY_NO_ANIMATION);
        intent.putExtra(AppConstant.REMINDER, note.convertToString());
        PendingIntent pendingIntent = PendingIntent.getBroadcast(getApplicationContext(), note.getId(), intent,
    ...
    private void saveInGoogleDrive() {
        ...
        if (mBundle != null) {
            sTmpFlNm = mBundle.getString(AppConstant.TEMP_FILE_NAME);
        }
        final String resourceId = AppConstant.NOTE_PREFIX + GDUT.time2Title(null) + AppConstant.JPG;
        ...
        ContentValues values = createContentValues(AppConstant.NOTE_PREFIX + GDUT.time2Title(null) + AppConstant.JPG,
    ...
    protected void onSaveInstanceState(Bundle outState) {
        outState.putString(AppConstant.TEMP_FILE_NAME, sTmpFlNm);
    ...
    private void moveToArchive(boolean isList) {
        ...
        TextView dateTime = (TextView) findViewById(R.id.time_text_view_make_note);
    ...
    private void editForSaveInGoogleDrive() {
        ...
        ContentValues values = createContentValues(AppConstant.NOTE_PREFIX + GDUT.time2Title(null) +
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            ...
            case AppConstant.REQ_SCAN: {
                if (resultCode == Activity.RESULT_OK) {
                    final String titl = GDUT.time2Title(null);
        ...
        if (requestCode == CAMERA_REQUEST && resultCode == RESULT_OK) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                Cursor cursor = getContentResolver().query(selectdImage, filePAthColumn, null, null, null);
                cursor.moveToFirst();
                int columnIdex = cursor.getColumnIndex(filePAthColumn[0]);
                mImagePath = cursor.getString(columnIdex);
                cursor.close();
    ...
    private Account showAccountPicker() {
        Account account = GDUT.AM.getPrimaryAccount(false);
